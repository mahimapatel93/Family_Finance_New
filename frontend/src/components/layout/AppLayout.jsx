...
import {
  ...
  BarChart2, Sparkles, Globe, LogOut, Menu, X, ChevronRight, Newspaper,
} from 'lucide-react';

const NAV_ITEMS = [
  { to: '/dashboard',   label: 'Dashboard',    icon: LayoutDashboard, tour: 'dashboard'   },
  { to: '/expenses',    label: 'Expenses',      icon: Receipt,         tour: 'expenses'    },
  { to: '/bills',       label: 'Bills',         icon: CreditCard,      tour: 'bills'       },
  { to: '/investments', label: 'Investments',   icon: TrendingUp,      tour: 'investments' },
  { to: '/reports',     label: 'Reports',       icon: BarChart2,       tour: 'reports'     },
  { to: '/ai-insights', label: 'AI Insights',   icon: Sparkles,        tour: 'ai-insights' },
  { to: '/market',      label: 'Market',        icon: Globe,           tour: 'market'      },
  { to: '/news',        label: 'News',          icon: Newspaper,       tour: 'news'        },
];
...
export default function AppLayout() {
  ...
  return (
    <div className="flex h-screen bg-surface-950 overflow-hidden">
      {/* Onboarding tour — renders only when user.isFirstLogin is true */}
      <OnboardingTour />
